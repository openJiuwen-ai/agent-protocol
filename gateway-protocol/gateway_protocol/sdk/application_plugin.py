...
"""应用插件（Application Plugin）扩展契约.

来源：``jiuwenswarm/extensions/sdk/application_plugin.py``。
``ApplicationPluginServices`` / 贡献类 / 基类为纯契约（stdlib only）；
宿主侧绑定细节（channel 包装等）由 gateway 仓实现持有。
"""
