...
"""目标 AgentServer 的受认证 HTTP bridge 基址解析与上传执行（纯静态函数）。

来源：``jiuwenswarm/common/client/agent_http_bridge.py``（common/client 终态
两侧删除后，本模块为唯一实现源）。

与原实现的差异（边界需要）：
- ``upload_file_bytes`` 原地 import jiuwenswarm 的 ``generate_file_upload_token``
  构成反向依赖；本包改为 ``token_factory`` 注入参数，由调用方（两侧各自的
  web 通道）传入自己的 token 生成函数，HTTP 转发逻辑保持一致。
- E2A 分片上传变体（``upload_file_bytes_via_e2a``）依赖 Gateway 的 E2A 路由，
  不进本包，留在 gateway 仓。
...
def upload_file_bytes(
    ...
    *,
    ...
    - ``target_rel_path`` 为相对用户目录根的目标路径（如
      ``agent/workspace/feishu_files/downloads/images/a.png``），与上传 token
      的 payload 语义一致；
    - ``token_factory`` 由调用方注入（两侧各自的 ``generate_file_upload_token``），
      本包不依赖任何实现侧 token 生成；
    ...
    token = token_factory(str(target_rel_path))
    base = (
