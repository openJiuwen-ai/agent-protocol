...
DEMO_LLM_CONFIG = {
    "api_key": os.getenv("ACP_DEMO_API_KEY", "").strip(),
    "base_url": os.getenv("ACP_DEMO_BASE_URL", "").strip(),
    "model": os.getenv("ACP_DEMO_MODEL", "").strip(),
    "temperature": os.getenv("ACP_DEMO_TEMPERATURE", "").strip(),
    "timeout": os.getenv("ACP_DEMO_TIMEOUT", "").strip(),
}
...
@dataclass
class HierarchicalGroupConfig:
    group_id: str
    leader_agent_id: str
    max_agents: int = 16


def _event_to_agent_inputs(event: Event) -> Dict[str, Any]:
    content = getattr(event, "content", None)
    context = getattr(event, "context", None)
    inputs = {
        "query": content.get_query() if hasattr(content, "get_query") else "",
        "conversation_id": getattr(context, "conversation_id", "default_session"),
    }
    extensions = getattr(content, "extensions", None)
    if isinstance(extensions, dict):
        inputs.update(extensions)
    return inputs


class _HierarchicalGroupController:
    def __init__(self, group: "HierarchicalGroup"):
        self._group = group

    async def send_to_agent(self, event: Event, agent_id: str, session: Any) -> Any:
        agent = self._group.get_agent(agent_id)
        return await agent.invoke(_event_to_agent_inputs(event))


class HierarchicalGroup:
    """Small in-demo group wrapper used by the ACP hierarchical demo."""
    ...
    def __init__(self, config: HierarchicalGroupConfig):
        ...
        self.group_controller = _HierarchicalGroupController(self)

    def add_agent(self, agent_id: str, agent: Any) -> None:
        if len(self._agents) >= self.config.max_agents:
            raise ValueError(f"Group {self.group_id} cannot contain more than {self.config.max_agents} agents.")
        self._agents[agent_id] = agent

        controller = getattr(agent, "controller", None)
        if controller is not None and hasattr(controller, "set_group"):
            controller.set_group(self)

    def get_agent(self, agent_id: str) -> Any:
        if agent_id not in self._agents:
            raise KeyError(f"Agent '{agent_id}' is not registered in group '{self.group_id}'.")
        return self._agents[agent_id]

    async def stream(self, event: Event) -> AsyncIterator[Any]:
        leader = self.get_agent(self.leader_agent_id)
        async for chunk in leader.stream(_event_to_agent_inputs(event)):
            yield chunk
...
def _build_demo_acp_client() -> ACPClient:
    api_key = str(DEMO_LLM_CONFIG.get("api_key", "")).strip()
    if not api_key:
        raise RuntimeError("ACP_DEMO_API_KEY must be set to run the demo.")

    model = str(DEMO_LLM_CONFIG.get("model", "")).strip()
    if not model:
        raise RuntimeError("ACP_DEMO_MODEL must be set to run the demo.")

    from openai import OpenAI

    client_kwargs = {"api_key": api_key, "timeout": _get_float_env("ACP_DEMO_TIMEOUT", 60.0)}
    ...
    return ACPClient(
        ...
        model=model,
        temperature=_get_float_env("ACP_DEMO_TEMPERATURE", 0.1),
    )
