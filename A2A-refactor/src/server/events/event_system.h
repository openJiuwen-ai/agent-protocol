...
constexpr EventType operator|(EventType lhs, EventType rhs) noexcept
{
...
constexpr EventType operator~(EventType v) noexcept
{
    return static_cast<EventType>(~static_cast<int>(v));
}
...
public:
     ...
     * @return true on success, false on failure.
     */
    static bool ReadEventFd(int fd, uint64_t& outValue);
     ...
     * @return Pointer to the event_base structure.
     */
    [[nodiscard]] event_base* GetEventBase() const;
