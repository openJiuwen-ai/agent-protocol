...
protected:
    ...
    // 辅助函数：创建echo工具的回调函数
    std::function<Mcp::CallToolResult(const Mcp::ServerContext&, const std::string&, const std::string&)>
    CreateEchoToolFunction()
    {
        return [](const Mcp::ServerContext & /* ctx */, const std::string &name, const std::string &arguments)
            -> Mcp::CallToolResult {
