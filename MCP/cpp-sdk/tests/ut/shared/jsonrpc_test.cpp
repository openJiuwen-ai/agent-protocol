...
using Mcp::PromptListChangedNotification;
using Mcp::RequestParams;
using Mcp::ResourceListChangedNotification;
using Mcp::ResourceUpdatedNotification;
using Mcp::ResourceUpdatedNotificationParams;
using Mcp::ToolListChangedNotification;
...
static constexpr std::size_t EXPECTED_ROOTS_COUNT = 2;
...
TEST_F(JSONRPCTest, JSONRPCRequestRootsListSerializationAndDeserializationSuccess)
{
    JSONRPCRequest req;
    req.id_ = REQUEST_ID;
    ...
    JSONRPCRequest req2;
    int rc = req2.Deserialize(serialized);
    EXPECT_EQ(0, rc);
    EXPECT_EQ(JSONRPC_VERSION, req2.jsonrpc_);
    EXPECT_EQ(RequestId(REQUEST_ID), req2.id_);
...
TEST_F(JSONRPCTest, JSONRPCRequestSerializationInitializeIncludesClientCapabilities)
{
...
TEST_F(JSONRPCTest, JSONRPCRequestDeserializationInitializeParsesClientCapabilities)
{
    ...
    JSONRPCRequest req;
    int rc = req.Deserialize(jsonStr);

    EXPECT_EQ(0, rc);
    EXPECT_EQ(JSONRPC_VERSION, req.jsonrpc_);
    EXPECT_EQ(RequestId(REQUEST_ID), req.id_);
...
TEST_F(JSONRPCTest, JSONRPCRequestSamplingCreateMessageSerializationAndDeserializationSuccess)
{
    JSONRPCRequest req;
    req.id_ = REQUEST_ID;
...
TEST_F(JSONRPCTest, JSONRPCRequestSamplingCreateMessageFallbackWhenContentMissingOrWrongType)
{
...
TEST_F(JSONRPCTest, JSONRPCResponseSamplingCreateMessageSerializationAndDeserializationSuccess)
{
...
TEST_F(JSONRPCTest, JSONRPCResponseSerializationInitializeIncludesServerCapabilities)
{
    JSONRPCResponse resp;
    resp.id_ = REQUEST_ID;
...
TEST_F(JSONRPCTest, JSONRPCResponseDeserializationInitializeParsesServerCapabilities)
{
    std::string jsonStr = R"({
        "jsonrpc": "2.0",
        "id": 1,
...
TEST_F(JSONRPCTest, JSONRPCResponseSerializationListRoots)
{
    JSONRPCResponse resp;
    resp.id_ = REQUEST_ID;
...
TEST_F(JSONRPCTest, JSONRPCResponseDeserializationListRoots)
{
    std::string jsonStr = R"({
        "jsonrpc": "2.0",
        "id": 1,
...
TEST_F(JSONRPCTest, ResourceUpdatedNotificationSerializeIncludesUriParam)
{
    JSONRPCNotification notif;
    notif.method_ = "notifications/resources/updated";

    auto payload = std::make_unique<ResourceUpdatedNotification>();
    payload->method_ = notif.method_;
    payload->params_ = std::make_unique<ResourceUpdatedNotificationParams>(RESOURCE_URI);
...
TEST_F(JSONRPCTest, ResourceUpdatedNotificationDeserializeParsesUriParam)
{
    json j;
    j["jsonrpc"] = JSONRPC_VERSION;
    ...
    JSONRPCNotification notif;
    int result = notif.Deserialize(j.dump());
    EXPECT_EQ(0, result);
    ...
    const auto *typed = dynamic_cast<const ResourceUpdatedNotification *>(notif.notification_.get());
    ASSERT_NE(nullptr, typed);
    ASSERT_NE(nullptr, typed->params_);
    const auto *params = dynamic_cast<const ResourceUpdatedNotificationParams *>(typed->params_.get());
    ASSERT_NE(nullptr, params);
    EXPECT_EQ(std::string{RESOURCE_URI}, params->uri);
}

TEST_F(JSONRPCTest, ToolListChangedNotificationRoundTripDeserializeTyped)
{
    json j;
    j["jsonrpc"] = JSONRPC_VERSION;
    ...
    EXPECT_EQ(0, result);
    ASSERT_NE(nullptr, notif.notification_);
    EXPECT_NE(nullptr, dynamic_cast<const ToolListChangedNotification *>(notif.notification_.get()));
}

TEST_F(JSONRPCTest, PromptListChangedNotificationSerializeIncludesParams)
{
    JSONRPCNotification notif;
...
TEST_F(JSONRPCTest, PromptListChangedNotificationDeserializeCreatesTypedNotification)
{
    json j;
    j["jsonrpc"] = JSONRPC_VERSION;
    ...
    EXPECT_EQ(0, result);
    ASSERT_NE(nullptr, notif.notification_);
    EXPECT_NE(nullptr, dynamic_cast<const PromptListChangedNotification *>(notif.notification_.get()));
}

TEST_F(JSONRPCTest, ResourceListChangedNotificationSerializeIncludesParams)
{
    JSONRPCNotification notif;
...
TEST_F(JSONRPCTest, ResourceListChangedNotificationDeserializeCreatesTypedNotification)
{
    json j;
    j["jsonrpc"] = JSONRPC_VERSION;
    ...
    EXPECT_EQ(0, result);
    ASSERT_NE(nullptr, notif.notification_);
    EXPECT_NE(nullptr, dynamic_cast<const ResourceListChangedNotification *>(notif.notification_.get()));
}
...
TEST_F(JSONRPCTest, RootsListChangedNotificationSerializeIncludesParams)
{
    JSONRPCNotification notif;
...
TEST_F(JSONRPCTest, RootsListChangedNotificationDeserializeCreatesTypedNotification)
{
    json j;
    j["jsonrpc"] = JSONRPC_VERSION;
