...
namespace {
    ServerTool CreateTestTool(const std::string& name = "test_tool") {
        ServerTool tool;
        auto echoFunc = [](const ServerContext & /* ctx */, const std::string &name, const std::string &arguments)
            -> Mcp::CallToolResult
        {
...
TEST(McpServerImplementTest, PromptManagement_AddPrompt)
{
    ...
    EXPECT_NO_THROW(server.AddPrompt(prompt.name,
                                    [](const ServerContext & /* ctx */, const std::string &promptName,
...
TEST(McpServerImplementTest, ResourceManagement_AddResource)
{
    ...
    Mcp::ReadResourceFunc readResourceFunc = [](const ServerContext & /* ctx */, const std::string &uri)
