...
static int RunToolDemo(const std::string& endpoint)
{
    ...
    try {
        ...
        std::function<void(double, std::optional<double>, const std::optional<std::string>&)> progressCallback =
            ...
            MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("Progress: ") + std::to_string(progress) + " (" +
                std::to_string(progress * 100) + "%)");
            
            if (message) {
                MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("Message: ") + *message);
            }
        };
        
        auto progressCallFuture = mcpClient->CallTool("progress_tool", progressArguments.dump(), 60, progressCallback);
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("CallTool with progress callback success, isError: ") +
            std::to_string(progressCallResult->isError) + ", content count: " +
            std::to_string(progressCallResult->content.size()));
        for (const auto &content : progressCallResult->content) {
            if (std::holds_alternative<Mcp::TextContent>(content)) {
                const auto &text = std::get<Mcp::TextContent>(content);
                MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("  TextContent: ") + text.text);
            }
        }
    } catch (const std::exception &e) {
        MCP_LOG(MCP_LOG_LEVEL_ERROR, std::string("CallTool with progress callback failed: ") + e.what());
        return -1;
    }
    ...
    try {
        ...
        auto asyncCallFuture = mcpClient->CallTool("async_echo", asyncArguments.dump(), 30);
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("Async CallTool success, isError: ") +
            std::to_string(asyncCallResult->isError) + ", content count: " +
            std::to_string(asyncCallResult->content.size()));
        for (const auto &content : asyncCallResult->content) {
            if (std::holds_alternative<Mcp::TextContent>(content)) {
                const auto &text = std::get<Mcp::TextContent>(content);
                MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("  Async TextContent: ") + text.text);
            }
        }
    } catch (const std::exception &e) {
        MCP_LOG(MCP_LOG_LEVEL_ERROR, std::string("Async CallTool failed: ") + e.what());
        return -1;
    }
    ...
    try {
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO,
            std::string("Complete success, completion count: ") +
            std::to_string(completeResult->completion.values.size()));
        for (const auto &completion : completeResult->completion.values) {
            MCP_LOG(MCP_LOG_LEVEL_INFO, std::string("  Suggestion: ") + completion);
        }
    } catch (const std::exception &e) {
        MCP_LOG(MCP_LOG_LEVEL_ERROR, std::string("Complete failed: ") + e.what());
        return -1;
    }
