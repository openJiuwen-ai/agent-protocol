...
static void LogSamplingMessage(const Mcp::SamplingMessage& msg)
{
    ...
    MCP_LOG(MCP_LOG_LEVEL_INFO, "  Message [%s]:", roleStr.c_str());
    ...
    std::visit([](auto&& content) {
        ...
        if constexpr (std::is_same_v<T, Mcp::SamplingMessageContentBlock>) {
            ...
            MCP_LOG(MCP_LOG_LEVEL_INFO, "    Content blocks count: %zu", content.size());
        }
    },
        msg.content);
}
    ...
    const Mcp::CreateMessageParams& params)
{
    ...
    MCP_LOG(MCP_LOG_LEVEL_INFO, "  maxTokens: %ld", params.maxTokens);
...
static std::string ParseEndpointFromArgs(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            MCP_LOG(MCP_LOG_LEVEL_INFO, "Usage: %s [--port=<1-65535>]",
                (argv[0] ? argv[0] : "SamplingExample"));
            exit(0);
        }
...
static bool InitializeClient(std::shared_ptr<Mcp::McpClient> client)
{
    ...
    try {
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, "  Server name: %s", initResult->serverInfo.name.c_str());
        MCP_LOG(MCP_LOG_LEVEL_INFO, "  Server version: %s", initResult->serverInfo.version.c_str());
        return true;
...
static bool ListTools(std::shared_ptr<Mcp::McpClient> client)
{
    ...
    try {
        ...
        for (const auto& tool : listResult->tools) {
            MCP_LOG(MCP_LOG_LEVEL_INFO, "  Tool: %s", tool.name.c_str());
            MCP_LOG(MCP_LOG_LEVEL_INFO, "    Description: %s", tool.description.c_str());
        }
        return true;
    } catch (const std::exception& e) {
...
static void LogToolResultContent(const Mcp::ContentType& content)
{
    if (std::holds_alternative<Mcp::TextContent>(content)) {
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, "  Response: %s", text.text.c_str());
    } else if (std::holds_alternative<Mcp::ImageContent>(content)) {
        const auto& image = std::get<Mcp::ImageContent>(content);
        MCP_LOG(MCP_LOG_LEVEL_INFO, "  Image: mimeType=%s", image.mimeType.c_str());
    }
}
...
static bool CallSamplingEchoTool(std::shared_ptr<Mcp::McpClient> client)
{
    ...
    try {
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, "Calling sampling_echo with prompt: '%s'",
            arguments["prompt"].get<std::string>().c_str());

        auto callFuture = client->CallTool(SAMPLE_TOOL_NAME, arguments, DEFAULT_MAX_TOKENS);
        ...
        MCP_LOG(MCP_LOG_LEVEL_INFO, "");
        MCP_LOG(MCP_LOG_LEVEL_INFO, "CallTool complete, isError: %d, content count: %zu",
            callResult->isError, callResult->content.size());
