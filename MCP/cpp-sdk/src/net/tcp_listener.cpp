...
TcpListener::TcpListener(Mcp::EventSystem& es) : es_(es)
{
}
    ...
    for (addrinfo* ai = res; ai; ai = ai->ai_next) {
        ...
        if (reusePort) {
            if (!setReusePort(fd, true)) {
                MCP_LOG(MCP_LOG_LEVEL_INFO, "requested SO_REUSEPORT not available on this platform");
            }
        }
...
bool TcpListener::Start()
{
    ...
    evAcceptId_ = es_.AddEvent(listenFd_, Mcp::EventType::READ, [this](int, short, void*) { HandleAccept(); }, this, 0);
