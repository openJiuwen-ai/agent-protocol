...
6. 只输出一个 JSON 对象，字段为 decision、reason、offerType、message，可选 updateSuggestion。不要输出 uiStyle：展示方式由 Host 根据 offerType 决定。offer_help + text_assistance 必须输出 helpTopic：选项反复切换用 compare_options，规则不清用 explain_rules，连续失败用 fix_block，只有确需保存或稍后继续时才用 save_progress。
