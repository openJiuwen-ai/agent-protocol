...
- {onPacket, transport?: never}：Host 托管；
- {transport, onPacket?: never}：SDK 托管；
- 二者都省略：只生成 packet，由调用方手工获取。

两种模式共享 policy、presenter、onAccept、feedbackUpload、诊断回调和时钟。
...
- coerceModelObject(value)：从合法 JSON 或包含完整 JSON 对象的模型文本恢复对象。
