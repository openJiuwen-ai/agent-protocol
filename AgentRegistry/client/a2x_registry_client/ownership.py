...
class OwnershipStore:
    ...
    def datasets(self) -> list[str]:
        """Return all datasets the client has owned services in (snapshot copy)."""
        with self._lock:
            return list(self._data.keys())

    def list(self, dataset: str) -> list[str]:
        """Return owned sids in ``dataset`` (snapshot copy; empty if unknown)."""
        with self._lock:
            return list(self._data.get(dataset, set()))
