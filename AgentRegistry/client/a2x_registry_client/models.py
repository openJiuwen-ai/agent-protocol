...
@dataclass
class PrincipalCreateResponse(_FromDictMixin):
    """Response from admin POST /api/auth/principals.

    ``token`` is the **plaintext** API key — the only moment it appears
    on the wire. The caller is expected to deliver it out-of-band (vault,
    secrets manager, encrypted IM) to the new principal. The server only
    keeps a sha256 hash from this point forward.
    """
...
class RegisterResponse(_FromDictMixin):
    ...
    dataset: str
    status: str
    # Populated when the server granted a heartbeat lease (i.e. the
    # namespace's ``lease_config.enabled=true`` AND the client passed
    # ``lease_ttl`` on register). ``None`` for permanent / legacy
    # services so SDK code that doesn't care can ignore these.
    lease_ttl: int | None = None             # seconds; same as request value
    lease_expires_at: float | None = None    # unix wall-clock
