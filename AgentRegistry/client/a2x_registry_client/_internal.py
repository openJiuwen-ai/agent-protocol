...
AUTH_PRINCIPALS_ROOT = "api/auth/principals"
...
def heartbeat_path(dataset: str, service_id: str) -> str:
    """Path for POST (extend lease) / DELETE (revoke lease) of a single sid."""
    return f"{DATASETS_ROOT}/{_encode(dataset)}/services/{_encode(service_id)}/heartbeat"


def lease_config_path(dataset: str) -> str:
    """Per-namespace heartbeat policy. GET public; POST admin-or-anon."""
    return f"{DATASETS_ROOT}/{_encode(dataset)}/lease-config"
...
def build_create_dataset_body(
    ...
    # auth_required is omitted when False so legacy callers produce byte-equal
    # request bodies; the backend treats missing == False.
    if auth_required:
        body["auth_required"] = True
    if lease_config is not None:
        body["lease_config"] = dict(lease_config)
    return body


def build_create_principal_body(
    handle: str,
    role: str,
    namespaces: list[str] | None,
    note: str | None = None,
) -> dict[str, Any]:
    """Body for POST /api/auth/principals. ``role`` must be admin / provider /
    user; ``namespaces`` is required for non-admin roles (backend rejects
    otherwise — surfaced as ``ValidationError``)."""
    body: dict[str, Any] = {"handle": handle, "role": role}
...
def apply_pagination(
    params: dict[str, Any], page: int, size: int
) -> dict[str, Any]:
    """Merge ``page`` / ``size`` into a query-param dict, with local validation.

    ``size == -1`` (default) → leave params untouched (backend returns all in
    one page). Otherwise the params get ``page`` and ``size`` as strings.
    Bool is rejected explicitly because ``isinstance(True, int)`` would
    otherwise let ``page=True`` slip through as ``page=1``.
    """
    if isinstance(page, bool) or not isinstance(page, int) or page < 1:
        raise ValueError(f"page must be int >= 1, got {page!r}")
    if isinstance(size, bool) or not isinstance(size, int) or size < -1:
        raise ValueError(f"size must be int >= -1 (-1 = all in one page), got {size!r}")
    if size != -1:
