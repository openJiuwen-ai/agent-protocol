...
def _migrate_legacy_image_table(conn: Any) -> None:
    ...
    Interrupted-run recovery, a previously crashed run may
    have left the ``image_migrated`` temp table behind — any leftover temp
    is resolved **before** migrating:

    - ``image`` table present → the temp is debris from an aborted run and
      the surviving original table is authoritative: the temp is dropped
      first (otherwise ``CREATE TABLE image_migrated`` collides with it
      and startup dies);
    - ``image`` table absent → the run died between DROP and RENAME and the
      data lives only in the temp: a temp carrying the new (name/version)
      layout is renamed into place (zero data loss); an unrecognisable
      temp is dropped as garbage.
    """
    cols = {
        row[1] for row in conn.execute("PRAGMA table_info(image)").fetchall()
    }
    temp_cols = {
        row[1] for row in conn.execute("PRAGMA table_info(image_migrated)").fetchall()
    }

    if not cols:
        # No image table: fresh database, or a migration interrupted after
        # DROP with the data stranded in the temp table.
        if {"name", "version"} <= temp_cols:
            conn.executescript(
                """
                BEGIN;
            ...
            conn.executescript(
                "BEGIN; DROP TABLE image_migrated; COMMIT;"
            )
        # fresh / recovered / cleaned — SCHEMA_SQL builds the rest
        return

    if temp_cols:
        # Original table survived: a leftover temp is stale debris (its
        # partial copy must NOT win). Drop it so the rebuild below cannot
        # collide with CREATE TABLE image_migrated.
        conn.executescript(
            "BEGIN; DROP TABLE image_migrated; COMMIT;"
        )
