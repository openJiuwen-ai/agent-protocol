...
def test_deregister_without_framework_cannot_be_in_use(image_svc: ImageService):
    """framework 为空且无实例引用（含 name 兜底无命中）时可直接删。"""
    _reg(image_svc, framework=None)
    result = image_svc.deregister("opencode", "v0.2.0")
    assert result["status"] == "deregistered"


def test_deregister_repo_stub_logs_real_imageurl(
    image_svc: ImageService, monkeypatch, caplog
):
    """注销的镜像仓删除 stub 取 runtime_spec.rootfs.imageurl（非顶层 imageurl）。

    data JSON 从未有顶层 imageurl——stub 曾读错位置导致日志恒为空串。
    """
    import logging as _logging

    monkeypatch.setenv("A2X_REGISTRY_REPO_BASE", "/repo")
    _reg(image_svc, runtime_spec=make_runtime_spec(
        imageurl="harbor.local/adapted/opencode:v0.2.0-mod1.3"))
    with caplog.at_level(_logging.INFO, logger="a2x_registry.image.service"):
        image_svc.deregister("opencode", "v0.2.0")
    stub = [r for r in caplog.records if "repo image deletion" in r.message]
    assert stub, "REPO_BASE 已配置，stub 日志应出现"
    assert "harbor.local/adapted/opencode:v0.2.0-mod1.3" in stub[0].getMessage()


def test_deregister_repo_stub_tolerates_missing_rootfs(
    image_svc: ImageService, monkeypatch, caplog
):
    """runtime_spec 缺 rootfs/imageurl（透传不校验）时 stub 优雅落空、不抛异常。"""
    import logging as _logging

    monkeypatch.setenv("A2X_REGISTRY_REPO_BASE", "/repo")
    _reg(image_svc, runtime_spec={"cpu": 1000})   # 无 rootfs
    with caplog.at_level(_logging.INFO, logger="a2x_registry.image.service"):
        result = image_svc.deregister("opencode", "v0.2.0")
    assert result["status"] == "deregistered"
    stub = [r for r in caplog.records if "repo image deletion" in r.message]
    assert stub and stub[0].getMessage().endswith("imageurl=")


def test_deregister_without_framework_blocked_by_name_fallback(image_svc: ImageService):
    """framework 为空：按镜像 name 兜底匹配在用实例，不做静默放行。

    实例行的 framework 必填，gateway 对 framework=None 的镜像只能填镜像
    标识（§7 模式 B 亦恒按 name 拆分）——early-return 会放过在用镜像。
    """
    _reg(image_svc, framework=None)  # name=opencode, version=v0.2.0
    image_svc._table_svc.register("instances", {
        "service_id": "generic_abc123",
        "kind": "三方",
        "framework": "opencode",   # 实例侧以镜像 name 作 framework 值
        "framework_version": "v0.2.0",
        "node": "node-1",
        "user": "user-01",
        "data": {},
...
def test_deregister_without_framework_fallback_requires_version_match(image_svc: ImageService):
    """name 兜底带 version 精确匹配：同名不同版本的实例不阻塞删除。"""
    _reg(image_svc, framework=None)
    image_svc._table_svc.register("instances", {
        "service_id": "generic_abc123",
        "kind": "三方",
        "framework": "opencode",
        "framework_version": "v0.1.0",   # 版本不匹配
        "node": "node-1",
        "user": "user-01",
        "data": {},
    })
